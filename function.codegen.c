// Autogenerated by maysick compiler.
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
maysick_any _mS_A_chan(m_string *s);
maysick_any _mS_A_hello(m_string *i);

maysick_any  _mS_A_chan(m_string *M__s) {
m_string*    M__s2;

m_arc       *arc_local = NULL;
maysick_any  retval    = m_any_nil();
M__s2 = m_to_string((m_arc_autorelease(&arc_local, (m_any_string(m_string_concat(M__s,m_arc_autorelease_s(&arc_local, m_string_from_cstr("ちゃん"))))))));
retval = m_arc_count((m_any_string(M__s2)));
goto DEFER;
DEFER:
m_arc_release(arc_local);
return retval;
}
maysick_any  _mS_A_hello(m_string *M__i) {

m_arc       *arc_local = NULL;
maysick_any  retval    = m_any_nil();
_mS__println(m_string_concat(m_to_string((m_arc_autorelease(&arc_local, _mS_A_chan(M__i)))),m_arc_autorelease_s(&arc_local, m_string_from_cstr("はすっごーいんだよ!"))));
DEFER:
m_arc_release(arc_local);
return retval;
}
...
m_arc_autorelease(&arc_local, _mS_A_hello(m_arc_autorelease_s(&arc_local, m_string_from_cstr("かばん"))));
m_arc_autorelease(&arc_local, _mS_A_hello(m_arc_autorelease_s(&arc_local, m_string_from_cstr("カラカル"))));
}
